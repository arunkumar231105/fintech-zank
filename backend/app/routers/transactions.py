...
def list_transactions(
    ...
    total = query.count()
    items = (
        query.order_by(TransactionModel.date.desc(), TransactionModel.id.desc())
        .offset((page - 1) * limit)
        .limit(limit)
        .all()
    )
...
def transaction_stats(
    ...
    transactions = (
        ...
        .filter(TransactionModel.user_id == current_user.id, TransactionModel.date >= start, TransactionModel.date <= end)
...
def export_transactions(
    ...
    rows = [_serialize_transaction(txn) for txn in transactions]
...
def get_transaction_detail(
    ...
    transaction = (
        db.query(TransactionModel)
        .filter(TransactionModel.user_id == current_user.id, TransactionModel.txn_id == transaction_id)
        .first()
    )
    if not transaction:
